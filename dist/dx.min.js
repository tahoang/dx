...
window.dx = _d, _d.define = function(e) {
    ...
    initialize: function() {
        dx.Component.prototype.initialize.apply(this, arguments), dx.log("Initialized model: " + this.name);
    },
    fields: {}
    ...
    initialize: function() {
        dx.Component.prototype.initialize.apply(this, arguments), dx.log("Initialized view: " + this.name);
    }
            ...
            if ("" != t.dataRoot && (e = e[t.dataRoot]), t.originalData = e, "" != t.model) {
                ...
                for (var o in e) {
                    ...
                    for (var r in n.fields) {
                        var d = n.fields[r], s = d.type, f = a[d.name];
                        switch (s) {
                          case "string":
                            i[d.name] = new String(f);
                            break;

                          case "int":
                            i[d.name] = new Number(f);
                            break;
    ...
    initialize: function() {
        ...
        if ("undefined" != typeof this.baseLayers) for (var d in this.baseLayers) {
            var s = this.baseLayers[d];
            void 0 !== s && (r[d] = s);
        }
        var f = {
            Overlays: this.geoJsonFeatures
        };
        if ("undefined" != typeof this.overlays) for (var d in this.overlays) {
            var s = this.overlays[d];
            f[d] = s;
        }
